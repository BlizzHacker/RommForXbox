...
Upload **`RommForXbox_0.9.0.0_x64_STORE.msixupload`** — in
...
`RommForXbox_0.9.0.0_x64_SIDELOAD.msix` beside it is the Dev Mode build, for
...
| Version | `0.9.0.0` (raise for every resubmission — the Store rejects a repeat) |
...
of which only hardware could reveal. 0.9.0.0 fixes them, but the two most
...
Ten minutes with `RommForXbox_0.9.0.0_x64_SIDELOAD.msix` through the Device
